...
(function () {
    ...
    let aimSparks = [];
    let liteFrame = 0;
    const isBdUniverse = () => document.body?.classList?.contains('bd-app');
    const isElectronApp = () => typeof window.blankDelay !== 'undefined';
    ...
    function initAll() {
        particles = [];
        const density = isBdUniverse() ? (isElectronApp() ? 22000 : 6500) : 9000;
        ...
        stars = [];
        const starCount = isBdUniverse() ? (isElectronApp() ? 220 : 780) : 620;
        for (let i = 0; i < starCount; i++) {
            ...
            stars.push({
                ...
                hue: isBdUniverse() ? 0 : (Math.random() < 0.35 ? 210 + Math.random() * 40 : Math.random() < 0.5 ? 280 + Math.random() * 30 : 0)
            });
        }
    ...
    setInterval(() => { if (Math.random() < (isBdUniverse() ? 0.65 : 0.45)) spawnMeteor(); }, isBdUniverse() ? 700 : 900);
    setInterval(() => { if (Math.random() < (isBdUniverse() ? 0.5 : 0.35)) spawnShooter(); }, isBdUniverse() ? 1100 : 1400);
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (isLicenseScreen()) return;
        if (isBdUniverse() && Math.random() < 0.35) {
            aimSparks.push({ x: mouseX, y: mouseY, vx: (Math.random() - 0.5) * 2, vy: (Math.random() - 0.5) * 2, life: 1 });
        }
    ...
    function drawNebula(spinCx, spinCy, t) {
        const aim = isBdUniverse();
        const maxR = Math.min(w, h) * (aim ? 0.98 : 0.72);
        const g = ctx.createRadialGradient(spinCx, spinCy, 0, spinCx, spinCy, maxR * 0.55);
        if (aim) {
            g.addColorStop(0, 'rgba(255, 255, 255, 0.1)');
            g.addColorStop(0.25, 'rgba(200, 200, 220, 0.06)');
            g.addColorStop(0.55, 'rgba(40, 40, 50, 0.04)');
        } else {
    ...
    function drawUniverse(t) {
        ...
        for (const s of stars) {
            ...
            ctx.beginPath();
            ctx.arc(x, y, s.size * (0.8 + tw * 0.4) * (aim ? 1.15 : 1), 0, Math.PI * 2);
            if (s.hue > 0 && !aim) {
                ctx.fillStyle = `hsla(${s.hue}, 55%, 75%, ${s.brightness * tw * 0.85})`;
            } else {
                const g = Math.floor((aim ? 160 : 140) + s.brightness * 115 * tw);
                ctx.fillStyle = `rgba(${g},${g},${Math.min(255, g + (aim ? 30 : 20))},${s.brightness * tw * (aim ? 0.95 : 0.8)})`;
            }
            ctx.fill();
        }
    }
    ...
    function drawMeteors() {
        for (let i = meteors.length - 1; i >= 0; i--) {
            ...
            if (isBdUniverse()) {
                ...
                g.addColorStop(0.25, `rgba(220,220,255,${m.life * 0.65})`);
            } else {
                g.addColorStop(0, `rgba(255,255,255,${m.life})`);
    ...
    function tick() {
        ...
        const g = ctx.createRadialGradient(mouseX, mouseY, 0, mouseX, mouseY, isBdUniverse() ? 420 : 320);
        if (isBdUniverse()) {
            g.addColorStop(0, `rgba(255,255,255,${0.14 + Math.abs(drift) * 0.08})`);
            g.addColorStop(0.4, `rgba(255,255,255,${0.05 + Math.abs(drift) * 0.04})`);
        } else {
        ...
        particles.forEach(p => {
            ...
            const gray = Math.floor((isBdUniverse() ? 120 : 50) + Math.sin(t+p.phase)*(isBdUniverse() ? 80 : 60));
            ctx.beginPath();
            ctx.arc(p.x, p.y, p.r * (isBdUniverse() ? 1.1 : 1), 0, Math.PI*2);
            ctx.fillStyle = `rgba(${gray},${gray},${gray},${p.a*tw*(isBdUniverse()?1.15:1)})`;
            ctx.fill();
        });

        for (let i = aimSparks.length - 1; i >= 0; i--) {
            const s = aimSparks[i];
            s.x += s.vx; s.y += s.vy; s.life -= 0.04;
            if (s.life <= 0) { aimSparks.splice(i, 1); continue; }
            ctx.beginPath();
        ...
        ripples.forEach((rip, i) => {
            ...
            ctx.strokeStyle = `rgba(255,255,255,${rip.life * (isBdUniverse() ? 0.5 : 0.45)})`; ctx.lineWidth = 2; ctx.stroke();
        });
